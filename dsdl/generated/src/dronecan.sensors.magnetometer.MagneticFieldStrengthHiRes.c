...
uint32_t _dronecan_sensors_magnetometer_MagneticFieldStrengthHiRes_encode(struct dronecan_sensors_magnetometer_MagneticFieldStrengthHiRes* msg, uint8_t* buffer
    ...
    __dronecan_sensors_magnetometer_MagneticFieldStrengthHiRes_encode(buffer, &bit_ofs, msg,
#if CANARD_ENABLE_TAO_OPTION
    tao
#else
    true
...
#endif
    uint32_t bit_ofs = 0;
    if (__dronecan_sensors_magnetometer_MagneticFieldStrengthHiRes_decode(transfer, &bit_ofs, msg,
#if CANARD_ENABLE_TAO_OPTION
    transfer->tao
#else
    true
